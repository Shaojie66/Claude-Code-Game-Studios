...
Uses `ask_user_dictation` at each phase transition. Delegates all file writes to
...
2. Phase 1b — `/ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `ask_user_dictation` confirms spec before review
3. Phase 1c — `/ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; `ask_user_dictation` confirms before Phase 3
...
4. `ask_user_dictation` presented with the specific flagged concerns and options:
...
- [ ] `ask_user_dictation` presents the specific flagged concerns before offering options
...
5. `ask_user_dictation` presents all three review results before Phase 5 begins
...
- [ ] `ask_user_dictation` shows all three review streams' results together before Phase 5 approval
...
3. `ask_user_dictation` presented with options:
...
- [ ] `ask_user_dictation` offers a "create pattern library first" option (referencing `/ux-design patterns`)
