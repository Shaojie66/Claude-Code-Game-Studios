...
- A small set of high-traffic workflows use hand-authored Codex-native wrappers instead of the generic bridge: `$studio-start`, `$studio-help`, `$studio-project-stage-detect`, `$studio-dev-story`, `$studio-brainstorm`, `$studio-design-system`, `$studio-code-review`, `$studio-story-readiness`, `$studio-story-done`, `$studio-setup-engine`, and `$studio-test-setup`.
