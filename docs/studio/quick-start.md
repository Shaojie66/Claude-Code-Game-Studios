...
This is a complete Game Studio CLI agent architecture for game development. It
...
In this fork, the active runtime surfaces are Codex-native:
use `$studio-*` skills and `/prompts:studio-*` prompts. Legacy slash-command
names shown below are upstream reference names, not the primary Codex entrypoints.
...
2. **Set up the engine** — Run `$studio-setup-engine` (uses the brainstorm recommendation)
   - Configures the Codex runtime guidance, detects knowledge gaps, populates reference docs
...
1. **Set up the engine** — Run `$studio-setup-engine [engine] [version]`
   (for example, `godot 4.6`) — also creates technical preferences
...
```
AGENTS.md                          -- Master Codex/OMX runtime contract
.codex/
  prompt-sources/studio/           -- Canonical studio role source material
  prompts/                         -- Generated `studio-*` prompts
  skills/                          -- Codex-native workflow definitions
docs/studio/
