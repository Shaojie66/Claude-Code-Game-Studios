...
2. **Ask architecture questions:**
   ...
   - *Use `ask_user_dictation` to batch constrained architecture questions*
...
Use `ask_user_dictation` for architecture decisions and next-step choices after
you explain the tradeoffs in conversation text.
...
**Example — architecture questions (batch):**
  ...
  ask_user_dictation with questions:
    1. "Where should DamageCalculator live: static utility (recommended), autoload singleton, or scene node?"
    2. "How should damage be rounded: floor to int (recommended), round to int, or keep decimal precision?"
```
