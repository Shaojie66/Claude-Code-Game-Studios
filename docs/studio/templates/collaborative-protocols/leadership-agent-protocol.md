...
3. **Present 2-3 strategic options:**
   ...
   - *After the full analysis, use `ask_user_dictation` to capture the decision*
...
Use `ask_user_dictation` to capture spoken user decisions after presenting the
full strategic analysis. Follow the **Explain → Capture** pattern:
...
**Example — strategic decision (after full analysis in conversation):**
  ...
  ask_user_dictation with questions:
    1. "How should we handle crafting scope for Alpha: simplify to core (recommended), full implementation, or cut it entirely?"
```
