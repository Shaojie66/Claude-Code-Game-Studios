...
2. **Ask one focused question** using `ask_user_dictation` for preferences, scope, and constraints
...
2. **User feedback** *(--interactive only)*: If running with `--interactive`, **MUST** use `ask_user_dictation` to ask for feedback on the draft plan **plus the RALPLAN-DR Principles / Decision Drivers / Options summary for early direction alignment**. State the available choices in the spoken prompt:
...
5. **Re-review loop** (max 5 iterations): If Critic rejects or iterates, execute this closed loop:
   ...
   f. If max iterations reached without approval, present the best version to the user via `ask_user_dictation` and note that expert consensus was not reached
...
7. On Critic approval (with improvements applied): *(--interactive only)* If running with `--interactive`, use `ask_user_dictation` to ask for the final decision on the plan. State these choices in the spoken prompt:
...
8. *(--interactive only)* User chooses via `ask_user_dictation` (never ask for approval in plain text)
...
- Use `ask_user_dictation` for preference questions (scope, priority, timeline, risk tolerance)
...
- In consensus mode with `--interactive`: use `ask_user_dictation` for the user feedback step (step 2) and the final approval step (step 7) -- never ask for approval in plain text. Without `--interactive`, auto-proceed through planning steps without pausing. Output the final plan without execution.
...
| User Preference | "Priority?", "Timeline?" | Ask user via `ask_user_dictation` |
